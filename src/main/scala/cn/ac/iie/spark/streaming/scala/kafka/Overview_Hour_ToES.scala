...
//      .set("spark.cores.max", properties.getProperty("hour.cores.max"))
    ...
    //val sc = new StreamingContext(conf, Seconds(50))
    val sc = new SparkContext(conf)
    val context = new SQLContext(sc)
    import context.implicits._
    val spark: SparkSession = SparkSessionSingleton.getInstance(conf)
    //val resultDF: RDD[collection.Map[String, AnyRef]] = sc.esRDD("ifp_cache_"+ CacheSourceTools.getNowDate() + "_small/TB_DW_Cache_Overview_5m","?q=100").values
    /*    values.foreach(line=>{
      val string: String = line.values.toString()
    })*/
...
//    val loadDF = context.read.options(options).format("org.elasticsearch.spark.sql").load("ifp_cache_"+time._1+"_small/TB_APP_Cache_Overview_5m")//TB_APP_Cache_Overview_5m
    val loadDF =context.esDF("ifp_cache_"+time._1+"_small/TB_APP_Cache_Overview_5m","{\"query\":{\"bool\":{\"must\":[{\"prefix\":{\"business_time\":\""+time._2+"\"}},{\"term\":{\"business_time_type\":\"5m\"}}],\"must_not\":[],\"should\":[]}},\"from\":0,\"sort\":[],\"aggs\":{}}")
