...
import cn.ac.iie.spark.streaming.scala.base.{CacheOverview, TB_SA_Cache_Small, TB_SA_Cache_big}
...
object CacheSourceTools {
  ...
  def getFiveDate(date: String): String = {
    val sdate = date.split(":")
    val df: SimpleDateFormat = new SimpleDateFormat("yyyyMMddHH:mm")
    if (sdate.length == 2) {
      val d = sdate(0).toString()
      val minute = sdate(1).toInt / 5f
      val mi = minute match {
  ...
  def unapplyToLong(str: String): (Long) = {
    try {
      str.toLong
    } catch {
      case e: NumberFormatException => 0
  ...
  def unapplyToInt(str:String):(Int)={
    try {
      str.toInt
    } catch {
      case e: NumberFormatException => 0
...
//      reqLines(0),
      ...
      reqLines(29),
      //回源Max-age
      ""+unapplyToLong(reqLines(30)),
      //回源文件大小
      ""+unapplyToLong(reqLines(31)),
      //缓存保存状态
      reqLines(32),
      //回源服务端口
      ""+unapplyToLong(reqLines(33)),
      //回源流量
      ""+unapplyToLong(reqLines(34)),
      //回源请求开始时间
      reqLines(35),
      //回源请求结束时间
      reqLines(36),
  ...
  def getSACacheInfo(lines:Array[String]
    ...
    var webSite = WebSiteInfo.getWebSite(domainInfo,ipwebinfoArray)
