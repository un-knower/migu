...
/**
  ...
  */
object ServerInfo_ToES_Big {
  def main(args: Array[String]): Unit = {
    val time=DateUtil.getUpdateTime("")
    Logger.getLogger("org.apache.spark").setLevel(Level.ERROR)
    val spark = SessionUtil.createSparkSession("ServerInfo_ToES")
    val date = DateUtil.getYesterdayTime(args(0))
    spark.sql("use ifp_source")


    val df=spark.sql("select '1' as rowid,cache_eqp_info as server_ip," +
    ...
    val df_temp01=spark.sql("select rowid,server_ip,sum(all_num)as all_num," +
      ...
      "update_datetime,business_time_type,business_time from temp01 " +
      "group by rowid,server_ip,flowunit,update_datetime,business_time_type,business_time")
    df_temp01.createOrReplaceTempView("temp02")
    val df_big=spark.sql("select rowid,server_ip,all_num,req_ok_num,req_fail_num," +
      ...
      "update_datetime,business_time_type,business_time from temp02")
    EsSparkSQL.saveToEs(df_big,"ifp_cache_"+date+"_big/TB_APP_Cache_ServerInfo_1d")
