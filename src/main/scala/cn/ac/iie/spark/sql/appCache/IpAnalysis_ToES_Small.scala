...
/**
  ...
  */
object IpAnalysis_ToES_Small {
  def main(args: Array[String]): Unit = {
    val time=DateUtil.getUpdateTime("")
    Logger.getLogger("org.apache.spark").setLevel(Level.ERROR)
    val spark = SessionUtil.createSparkSession("IpAnalysis_ToES_Small")
    val date = DateUtil.getYesterdayTime(args(0))
    spark.sql("use ifp_source")


    val df01=spark.sql("select '1' as rowid,factory,websiteid,websitename,cache_type,user_ip,all_num," +
    ...
    val df_small=spark.sql("select rowid,factory,webSiteID,webSiteName,cache_type,user_ip," +
      ...
      "update_datetime,business_time_type,business_time from temp11 group by rowid,factory" +
      ",websiteid,websitename,cache_type,user_ip,flowUnit,update_datetime,business_time_type,business_time")
    EsSparkSQL.saveToEs(df_small,"ifp_cache_"+date+"_small/TB_APP_Cache_ipanalysis_1d")
