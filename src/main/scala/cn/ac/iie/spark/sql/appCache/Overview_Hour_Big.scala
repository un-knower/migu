...
//    val loadDF = context.read.options(options).format("org.elasticsearch.spark.sql").load("ifp_cache_"+time._1+"_big/TB_APP_Cache_Overview_5m")
    val loadDF =context.esDF("ifp_cache_"+time._1+"_big/TB_APP_Cache_Overview_5m","{\"query\":{\"bool\":{\"must\":[{\"term\":{\"business_time_1h\":\""+getNowDateJustHH()+"\"}},{\"term\":{\"business_time_type\":\"5m\"}}],\"must_not\":[],\"should\":[]}},\"from\":0,\"sort\":[],\"aggs\":{}}")
    ...
    val sql = spark.sql("select 'huawei' as factory, '1h'as business_time_type" +
      ...
      " from caches  " +
      " group by cache_eqp_info,city,access_type,user_terminal,webSiteID,webSiteName  ")
