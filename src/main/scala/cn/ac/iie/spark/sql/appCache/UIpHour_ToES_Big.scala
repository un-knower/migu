...
/**
  ...
  */
object UIpHour_ToES_Big {
  def main(args: Array[String]): Unit = {
    val time=DateUtil.getUpdateTime("")
    Logger.getLogger("org.apache.spark").setLevel(Level.ERROR)
    val spark = SessionUtil.createSparkSession("UIpHour_ToES")
    val date = DateUtil.getYesterdayTime(args(0))
    spark.sql("use ifp_source")

    val df_big = spark.sql("select '1' as rowid,cache_type,webSiteID,webSiteName," +
      "business_hour,count(distinct(user_ip)) as userip_num,'"+time+"' as update_datetime" +
      ",'1d' as business_time_type,ds as business_time from TB_DW_Cache_big" +
      " where ds='"+date+"' group by cache_type,business_hour,webSiteID,webSiteName,ds")
    EsSparkSQL.saveToEs(df_big,"ifp_cache_"+date+"_big/TB_APP_Cache_uiphour_1d")
