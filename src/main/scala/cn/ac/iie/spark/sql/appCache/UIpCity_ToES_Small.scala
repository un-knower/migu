...
/**
  ...
  */
object UIpCity_ToES_Small {
  def main(args: Array[String]): Unit = {
    val time=DateUtil.getUpdateTime("")
    Logger.getLogger("org.apache.spark").setLevel(Level.ERROR)
    val spark = SessionUtil.createSparkSession("UIpCity_ToES_Small")
    val date = DateUtil.getYesterdayTime(args(0))
    spark.sql("use ifp_source")


    val df_small=spark.sql("select '1' as rowid,cache_type,webSiteID,webSiteName,city" +
      ",count(distinct(user_ip)) as userip_num,'"+time+"' as update_datetime,'1d' as business_time_type" +
      ",ds as business_time from TB_DW_Cache_small where ds = '"+date+"' " +
      "group by cache_type,webSiteID,webSiteName,city,ds")
    EsSparkSQL.saveToEs(df_small,"ifp_cache_"+date+"_small/TB_APP_Cache_uipcity_1d")
