...
//      .set("es.nodes", "10.0.30.101,10.0.30.102,10.0.30.105,10.0.30.107")
...
//    val time=getTime(timeSplit(0))
    var time=if(args.length!=1)getTime(timeSplit(0))else (timeSplit(0),args(0).toString)
    val loadDF: DataFrame = context.read.format("org.elasticsearch.spark.sql").load("ifp_cache_"+time._2+"_small/TB_APP_Cache_Overview_1h")
//        val loadDF: DataFrame = context.read.format("org.elasticsearch.spark.sql").load("ifp_cache_20180414_small/TB_APP_Cache_Overview_5m")
    loadDF.createOrReplaceTempView("caches")
    println("日期："+time._1+"读取"+time._2+"的数据")
    import spark.implicits._
    val sql = spark.sql("select domain,'huawei' as factory,'small' as cache_type,'' as file_type, '"+time._2+"' as business_time,'"+CacheSourceTools.getUpdateTime("")+"' as update_datetime,'Kb'as flowUnit,sum(all_flow) as all_flow ,avg(fstpack_delay) as fstpack_delay,sum(denied_flow) as denied_flow,avg(back_fstpack_delay) as back_fstpack_delay,sum(back_flow) as back_flow, '1d' as  business_time_type,cache_eqp_info,city,access_type,user_terminal,webSiteID,webSiteName,sum(all_num) as all_num,sum(other_flow) as other_flow,sum(req_ok_num) as req_ok_num,sum(req_fail_num) as req_fail_num,sum(req_Hit_num) as req_Hit_num,sum(req_Miss_num) as req_Miss_num,sum(req_Denied_num) as req_Denied_num,sum(req_Error_num) as req_Error_num,sum(return2xx_num) as return2xx_num,sum(return3xx_num) as return3xx_num,sum(return4xx_num) as return4xx_num,sum(return5xx_num) as return5xx_num,sum(returnerror_num) as returnerror_num,sum(req_get_num) as req_get_num,sum(req_post_num) as req_post_num,sum(req_other_num) as req_other_num,sum(hit_flow) as hit_flow,sum(miss_flow) as miss_flow,avg(down_speed) as down_speed,sum(back4xx_num) as back4xx_num,sum(back5xx_num) as back5xx_num,sum(dnsparse_fail_num) as dnsparse_fail_num,sum(backlink_fail_num) as backlink_fail_num,sum(nocache_flow) as nocache_flow,avg(back_downspeed) as back_downspeed from caches group by cache_eqp_info,city,access_type,user_terminal,webSiteID,webSiteName,domain ")
    EsSparkSQL.saveToEs(sql, "ifp_cache_" + time._2 + "_small/TB_APP_Cache_Overview_1d")
  }
  //根据当前时间加以判断返回（天，小时）
