...
object FromHDFSOverview {
  Logger.getLogger("org.apache.spark").setLevel(Level.ERROR)
  Logger.getLogger("org.eclipse.jetty.server").setLevel(Level.OFF)
//  val checkpointDirectory = "G://atest//Checkpoint_Data"
//  val checkpointDirectory = "hdfs://10.0.30.101:8020/Checkpoint_Data_Small"
  //广播变量
  @transient
...
//      .setMaster("local[3]")
    ...
    context.checkpoint(prop.getProperty("small.checkpointDirectory"))
    //WEB Host匹配规则
...
//    val DStream = context.textFileStream("hdfs://10.0.30.101:8020/user/hadoop-user/coll_data/cache_log/jiangxi/791/small/")
    val DStream = context.textFileStream(prop.getProperty("hdfs.small.streaming.path"))
      try {
        DStream.foreachRDD(lines=>{
          ...
          if(result.count()>0){
            EsSparkSQL.saveToEs(result, "ifp_cache_"+CacheSourceTools.getNowDate()+"_small/TB_APP_Cache_Overview_5m")
          }
...
//            var path ="hdfs://10.0.30.101:8020/ifp_source/DataCenter/SA/TB_SA_Cache_small/"+date+"/610000/"+utilss.getTime.replaceAll(":","")+"/data/store/"
            var path ="hdfs://"+prop.getProperty("hdfs.host")+":8020/ifp_source/DataCenter/SA/TB_SA_Cache_small/"+date+"/610000/"+utilss.getTime.replaceAll(":","")+"/data/store/"
